...
router.put('/like/:post_id', verifyToken, async (req, res) => {
	try {
		...
		const liked = post.likes.find((item) => item.user.toString() === req.body.user.id);
		if (liked) {
...
router.put('/unlike/:post_id', verifyToken, async (req, res) => {
	try {
		...
		const liked = post.likes.find((item) => item.user.toString() === req.body.user.id);
		if (!liked) {
			return res.status(400).send({ message: 'Post is not liked by this user.' });
		}
...
//add a comment
router.put('/comment/:post_id', [verifyToken, validatePost], async (req, res) => {
	try {
		const postId = req.params.post_id;
...
//delete a comment
router.delete('/comments/:post_id/:comment_id', verifyToken, async (req, res) => {
	try {
		const postId = req.params.post_id;
		...
		const post = await Post.findById(postId).populate('user', 'name avatar');
		const comment = post.comments.find((item) => item.id.toString() === commentId);
